...
const maxRetries = Envapter.getNumber('MAX_RETRIES', 3); // number
...
import { Envapter } from 'envapt';
// ---cut---
if (!Envapter.has('SENTRY_DSN')) {
    throw new Error('Set SENTRY_DSN in your .env before deploying.');
}
```
...
class Config extends Envapter {
    readonly maxUploadMb = this.getNumber('MAX_UPLOAD_MB', 25);
