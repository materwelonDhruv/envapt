...
const maxBytes = Envapter.getBigInt('MAX_UPLOAD_BYTES', 0n); // bigint
const key = Envapter.getSymbol('KEY', Symbol.for('envapt:key')); // symbol
```
...
const dsn = Envapter.resolve`postgres://${'PGUSER'}:${'PGPASSWORD'}@${'PGHOST'}/${'PGDATABASE'}`;
//    ^?
```
...
`Envapter` also detects the current runtime environment and exposes it through the `isProduction` / `isStaging` / `isDevelopment` / `isTest` helpers. On Node, Bun, and Deno it auto-loads the matching `.env` files for that environment. The helpers, the `Environment` enum, and per-environment file selection are documented on [Environment](/docs/environment).
