...
The fallback must itself be valid for the converter (a real email, an in-range port, a finite number, a valid `Date`). An invalid one throws `FallbackConverterTypeMismatch`.
