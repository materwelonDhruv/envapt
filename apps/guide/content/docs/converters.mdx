...
A fallback removes `undefined` from the return type, the same way it does for the [primitive readers](/docs/functional-api#fallbacks-shape-the-return-type). Without one, the return is `T | undefined`.
...
See [Fail-fast on missing values](/docs/functional-api#fail-fast-on-missing-values) for `require`, and [Standard Schema](/docs/standard-schema) for validating a value through zod, valibot, or arktype.
