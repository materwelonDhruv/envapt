...
Import from `envapt`. Its package export conditions resolve a node-free portable build on Workers automatically. The `envapt/workerd` subpath still resolves the same build but is deprecated and removed in v8.
...
Envapter.envPaths = ['.env']; // compiles, but throws FileApiUnsupported at runtime off Node
```

</Callout>

<Callout type="warn">
    `WorkerEnvSource` is a deprecated alias of `PortableSource`, removed in v8. Replace `new WorkerEnvSource(env)` with
    `new PortableSource(env)`.
</Callout>
