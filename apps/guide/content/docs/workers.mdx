...
import { Converters, Envapter, PortableSource } from 'envapt';

Envapter.useSource(new PortableSource(env));
Envapter.getUsing('CACHE_TTL', Converters.Time, '1m'); // readers work once a source is bound
...
Env vars and secrets are readable at module scope through `cloudflare:workers`, and they are constant for a deployment. Bind the source in a config module and export the typed values the Worker needs. Binding then happens once per isolate, and every request reads the exported values.
...
A Cloudflare `vars` entry can hold JSON as well as a string. `PortableSource` JSON-stringifies any non-string binding so the converters parse it the same way. A binding whose value stringifies to `undefined` is dropped, and anything you read this way must be JSON-serializable.
