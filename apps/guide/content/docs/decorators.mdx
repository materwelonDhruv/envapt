...
A module that imports decorators only from `envapt/legacy` must also `import 'envapt'` once so the runtime source binds. Runtime notes are in [Compatibility](/docs/compatibility).
