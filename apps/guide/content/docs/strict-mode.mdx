...
To fail fast on a specific value, use `require` or `getRequired`. These throw on an unset or empty value in any mode, and on a whitespace-only value under strict mode.
