...
<Callout type="tip">
    Reach for `required` / `require` when one specific value must be present. Reach for strict mode when you want
    stricter whitespace, template, and array-element rules everywhere.
</Callout>

See [Fail-fast on missing values](/docs/functional-api#fail-fast-on-missing-values) for the reader API and [Errors](/docs/errors) for the codes.
