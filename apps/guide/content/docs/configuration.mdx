...
<Callout type="warn">
    Everything on this page reads or writes the filesystem, so it applies to Node, Bun, and Deno only. The five file
    APIs (`envPaths`, `baseDir`, `envFileOptions`, `configureProfiles`, `resetProfiles`) are not available on Cloudflare
    Workers or in the browser; there you bind an environment source instead. See [Sources](/docs/sources).
</Callout>
...
<Callout type="info">
    `import 'envapt/config'` loads at import time. Set `ENVAPT_DEBUG=verbose` before the import to see
    that load logged. An `Envapter.debug` assignment in code runs after the load, so it is too late to
    log it.
</Callout>
