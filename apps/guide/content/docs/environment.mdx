...
`MODE` is last because Vite-family browser builds expose `import.meta.env.MODE` but none of the others, so binding `new ManualEnvSource(import.meta.env)` lets the same helpers work there. See [Browser](/docs/browser).
...
When your per-environment files live elsewhere, `configureProfiles` layers explicit paths on top of the cascade for the environments you name. Earlier paths in the array win. `configureProfiles` and `resetProfiles` read the filesystem, so they are not available off Node; see [Sources](/docs/sources).
