...
Reading `GREETING` with no `NAME` set returns `hello ${NAME}`. Enable [debug mode](/docs/configuration#debug-mode) to log each unresolved reference. Under [strict mode](/docs/strict-mode), an unresolved reference throws `EnvaptError` with code `MissingEnvValue` instead of being left as text.

<Callout type="warn">
    In non-strict mode the placeholder reaches your consumer: the value still carries the literal `${VAR}` text rather
    than a resolved value. Turn on strict mode if a missing reference should be a hard failure instead.
</Callout>
