...
envapt runs on Node `>=20`, Bun `>=1.3`, and Deno `>=2.5`, plus Cloudflare Workers and the browser, where you bind an environment source yourself. The functional API needs no build step on any runtime; the decorator API is TypeScript and needs compilation, with one caveat on Bun. See [Compatibility](/docs/compatibility).
