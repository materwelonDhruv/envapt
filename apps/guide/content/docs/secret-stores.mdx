...
If the fetch cannot finish before that first read, write a custom source that implements `FileBackedSource` with `supportsFiles` set to `true`. It keeps the `.env` cascade running alongside your fetched values. See [Sources](/docs/sources).
