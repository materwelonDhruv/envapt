...
description: Moving from envapt v7 to v8. One universal envapt import replaces the workerd and browser subpaths, and the portable file APIs warn instead of throwing by default.
---

v8 is a major release with two breaking changes. The `envapt/workerd` and `envapt/browser` subpaths are removed, and on the portable build the filesystem-only config APIs warn once and no-op by default instead of throwing. Everything else (the `Envapter` readers, converters, sources, decorators, and `envapt/legacy`) is unchanged.
